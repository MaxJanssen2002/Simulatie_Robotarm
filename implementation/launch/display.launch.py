...
def generate_launch_description():
    ...
    implementation_path = FindPackageShare('implementation')
    default_model_path = PathJoinSubstitution(['urdf', '01-myfirst.urdf'])
    default_rviz_config_path = PathJoinSubstitution([implementation_path, 'rviz', 'urdf.rviz'])
    ...
    ld.add_action(DeclareLaunchArgument(name='model', default_value=str(default_model_path), description='Path to robot urdf file relative to implementation package'))


    ld.add_action(IncludeLaunchDescription(
        PathJoinSubstitution([FindPackageShare('urdf_launch'), 'launch', 'display.launch.py']),
        launch_arguments={
            'urdf_package': 'implementation',
