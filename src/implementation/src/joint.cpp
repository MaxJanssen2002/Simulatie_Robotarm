...
Joint::Joint(const signed char a_index,
...
void Joint::adjustGoal(const signed char a_index, const double newValue, const double duration)
{
    if (a_index != index)
    {
        return;
    ...
    goalPWM = MathUtils::map(newValue, MINIMUM_PWM, MAXIMUM_PWM, minimum, maximum);
